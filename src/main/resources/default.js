 ...
 */
// import 'package:pdfjs/shared/util.dart';
// import 'package:pdfjs/core/primitives.dart';
// import 'package:pdfjs/core/stream.dart';
// import 'package:pdfjs/core/parser.dart';
// import 'package:pdfjs/core/image.dart';
// import 'package:pdfjs/core/colorspace.dart';
// import 'package:pdfjs/core/murmurhash3.dart';
// import 'package:pdfjs/core/fonts.dart';
// import 'package:pdfjs/core/function.dart';
// import 'package:pdfjs/core/pattern.dart';
// import 'package:pdfjs/core/cmap.dart';
// import 'package:pdfjs/core/metrics.dart';
// import 'package:pdfjs/core/bidi.dart';
// import 'package:pdfjs/core/encodings.dart';
// import 'package:pdfjs/core/standard_fonts.dart';
// import 'package:pdfjs/core/unicode.dart';
// import 'package:pdfjs/core/glyphlist.dart';

class NativeImageDecoder {
    constructor(xref, resources, handler, forceDataSchema) {
        this.xref = xref;
     ...
     */
    static isSupported(image, xref, res) {
     ...
     */
    static isDecodable(image, xref, res) {
        var cs = ColorSpace.parse(image.dict.get('ColorSpace', 'CS'), xref, res);
...
class TimeSlotManager {
    ...
    check() {
        if (++this.checked < this.CHECK_TIME_EVERY) {
            return false;
        }
        this.checked = 0;
    ...
    reset() {
        this.endTime = Date.now() + this.TIME_SLOT_DURATION_MS;
        this.checked = 0;
    }
}

export class PartialEvaluator {
    private var DefaultPartialEvaluatorOptions = {
    ...
    constructor(pdfManager, xref, handler, pageIndex, uniquePrefix, idCounters, fontCache, options) {
        ...
        this.options = options || this.DefaultPartialEvaluatorOptions;
    }

    private var deferred = Promise.resolve();
    ...
    buildFormXObject(resources, xobj, smask, operatorList, task, initialState) {
        ...
        return this.getOperatorList(xobj, task,
            (xobj.dict.get('Resources') || resources), operatorList, initialState).then(function () {
    ...
    buildPaintImageXObject(resources, image, inline, operatorList, cacheKey, imageCache) {
        var self = this;
        var dict = image.dict;
        ...
        PDFImage.buildImage(self.handler, self.xref, resources, image, inline,
            nativeImageDecoder).then(function (imageObj) {
    ...
    handleSMask(smask, resources, operatorList, task, stateManager) {
    ...
    setGState(resources, gState, operatorList, task,
              xref, stateManager) {
                ...
                case 'Font':
                    promise = promise.then(function () {
                        return self.handleSetFont(resources, null, value[0], operatorList,
                            task, stateManager.state).then(function (loadedName) {
    ...
    handleColorN(operatorList, fn, args, cs, patterns, resources, task, xref) {
        ...
        if (isName(patternName) &&
            ...
            if (typeNum === this.TILING_PATTERN) {
                var color = cs.base ? cs.base.getRgb(args, 0) : null;
                return this.handleTilingType(fn, color, resources, pattern,
                    dict, operatorList, task);
            } else if (typeNum === this.SHADING_PATTERN) {
    ...
    getOperatorList(stream, task, resources, operatorList, initialState) {
    ...
    getTextContent(stream, task, resources, stateManager, normalizeWhitespace) {
    ...
    readCidToGidMap(cidToGidStream) {
    ...
    extractWidths(dict, xref, descriptor, properties) {
        var glyphsWidths = [];
        var defaultWidth = 0;
        var glyphsVMetrics = [];
        var defaultVMetrics;
    ...
    translateFont(preEvaluatedFont, xref) {
...
class OperatorList {
    var CHUNK_SIZE = 1000;
    var CHUNK_SIZE_ABOUT = this.CHUNK_SIZE - 5; // close to chunk size

    private getTransfers(queue) {
        var transfers = [];
    ...
    constructor(intent, messageHandler, pageIndex) {
    ...
    addOp(fn, args) {
        ...
        if (this.messageHandler) {
            if (this.fnArray.length >= this.CHUNK_SIZE) {
                this.flush();
            } else if (this.fnArray.length >= this.CHUNK_SIZE_ABOUT &&
    ...
    flush(lastChunk) {
        ...
        var transfers = this.getTransfers(this);
...
class EvaluatorPreprocessor {
    ...
    private var getOPMap = getLookupTableFactory(function (t) {
        // Graphic state
        ...
        t['d1'] = {
            id: OPS.setCharWidthAndBounds, numArgs: 6,
            variableArgs: false
        };
        ...
        t['BDC'] = {
            id: OPS.beginMarkedContentProps, numArgs: 2,
            variableArgs: false
        };
...
class QueueOptimizer {
    private addState(parentState, pattern, fn) {
    ...
    private handlePaintSolidColorImageMask(iFirstSave, count, fnArray,
                                           argsArray) {
    ...
    constructor() {
        ...
        this.addState(this.InitialState,
            ...
            function foundImageMaskGroup(context) {
                ...
                count = this.handlePaintSolidColorImageMask(iFirstSave, count, fnArray,
                    argsArray);
                ...
                if (isSameImage) {
                    ...
                    for (q = 0; q < count; q++) {
                        ...
                        images.push({
                            data: maskParams.data, width: maskParams.width,
    ...
    optimize(queue) {
        ...
        while (i < ii) {
            state = (state || this.InitialState)[fnArray[i]];
